proc addHingeBeam {elePos eleCode iNode jNode sec springMatId kRat release rhoName zV} {
	...
	set mNodes [manageFEData -getAlignedPos $elePos $eleCode]
    set iiNode $iNode
    set jjNode $jNode
	...
	foreach mNode $mNodes d $ds {
		incr i
		set mOfs [expr 0.5*($jntData($mNode,dim,$dir,np,h) + $jntData($iNode,dim,$dir,nn,h))*$inputs(rigidZoneFac)]
		if {$mOfs < 1e-3 && $iOfs < 1e-3} {
			set transTag $zerOffTransTag
		} else {
			set offsVeci($dir) $iOfs
			set offsVecj($dir) $mOfs
			set transTag [addGeomTransf "$eleCode,$elePos,$i" Linear $zAxis offsVeci offsVecj]
		}
