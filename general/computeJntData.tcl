...
for {set j 1} {$j <= $inputs(nFlrs)} {incr j} {
	...
	foreach dir "X Y" nGridX "$inputs(nBaysX) [expr $inputs(nBaysX)+1]" nGridY "[expr $inputs(nBaysY)+1] $inputs(nBaysY)" {
		for {set k 1} {$k <= $nGridY} {incr k} {
			for {set i 1} {$i <= $nGridX} {incr i} {
				...
				set H2 [expr $H/2.]
				updateVar -sum jntData($iNodePos,dim,$dir,pn,v) $H2
				updateVar -sum jntData($iNodePos,dim,$dir,pp,v) $H2
				updateVar -sum jntData($jNodePos,dim,$dir,nn,v) $H2
				updateVar -sum jntData($jNodePos,dim,$dir,np,v) $H2
				...
				# m
				if {$conf == "/\\" || $conf == "|"} {
					updateVar -sum jntData($mNodePos,dim,$dir,nn,h) [expr $lhJ/2.]
					updateVar -sum jntData($mNodePos,dim,$dir,nn,v) $lvJ
					updateVar -sum jntData($mNodePos,dim,$dir,pn,h) [expr $lhJ/2.]
					updateVar -sum jntData($mNodePos,dim,$dir,pn,v) $lvJ
				}
				# n
				if {$conf == "\\/" || $conf == "|"} {
					updateVar -sum jntData($nNodePos,dim,$dir,np,h) [expr $lhI/2.]
					updateVar -sum jntData($nNodePos,dim,$dir,np,v) $lvI
					updateVar -sum jntData($nNodePos,dim,$dir,pp,h) [expr $lhI/2.]
	...
	for {set k 1} {$k <= [expr $inputs(nBaysY)+1]} {incr k} {
		for {set i 1} {$i <= [expr $inputs(nBaysX)+1]} {incr i} {
			...
			#the roof joints also need pp dims for the beams connecting to them
			if {$j == $inputs(nFlrs)} {
				updateVar -sum jntData($jNodePos,dim,X,pp,h) [expr $dx/2.]
				updateVar -sum jntData($jNodePos,dim,X,np,h) [expr $dx/2.]
				updateVar -sum jntData($jNodePos,dim,Y,pp,h) [expr $dy/2.]
				updateVar -sum jntData($jNodePos,dim,Y,np,h) [expr $dy/2.]
			}
...
manageJntData -makeList
#set missing jnt dims. based on the computed ones
foreach pos [manageJntData -getAllPos] {
	# puts "pos = $pos"
	foreach dir "X Y" {
		foreach vrt "pp np nn pn" {
			foreach com "h v" {
				...
				set jntData($name) [manageJntData -getMatchingDim $pos $dir $vrt $com]
			}
		}
	}
}
