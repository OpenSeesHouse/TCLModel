...
set L1 [expr 0.50*(1.0*$inputs(deadFloor)+1*$inputs(liveFloor))*9.1*9.1*$g]
for {set j 1} {$j < $inputs(nFlrs)} {incr j} {
	set pntLoadList($j) "$L1 $L1 $L1 $L1"
}

set L1 [expr 0.50*(1.0*$inputs(deadRoof)+1*$inputs(deadRoof))*9.1*9.1*$g]
set j $inputs(nFlrs)
set pntLoadList($j) "$L1 $L1 $L1 $L1"
